...
DHT_VALUE DHT11::ReadDHT(){
  ...
  if(Read()){
    PRINTLN("Humidity : ");
    DecToBin(raw_humidity); PRINT("\t");
    humi = raw_humidity >> 8;
    DecToBin8(humi); PRINT("\t");
    raw_humidity = raw_humidity << 8;
    humid = raw_humidity >> 8;
    DecToBin8(humid); PRINT("\t");
    PRINT(humi); PRINT("."); PRINT(humid); PRINTLN("");

    PRINTLN("Temperature : ");
    DecToBin(raw_temperature); PRINT("\t");
    temp = raw_temperature >> 8;
    DecToBin8(temp); PRINT("\t");
    raw_temperature = raw_temperature << 8;
    tempd = raw_temperature >> 8;
    DecToBin8(tempd); PRINT("\t");
    PRINT(temp); PRINT("."); PRINT(tempd); PRINTLN("");

    PRINTLN("Checksum byte : ");
    PRINT("RECV : ");
    DecToBin8(check_sum);
    PRINTLN(""); PRINT("CALL : ");
    DecToBin8(temp + tempd + humi + humid); PRINTLN("");
